...
export function GroupNotes({ roomId, currentUserId }: { roomId: string; currentUserId: string }) {
  ...
  return <section aria-labelledby="group-notes-heading" className="mt-12 space-y-6">
    ...
    {data && <div className="space-y-4">
      {data.notes.length === 0 && <p className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground"><MinimalModeText short="No notes yet.">No notes yet. Add your first note.</MinimalModeText></p>}
